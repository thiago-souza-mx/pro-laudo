...
export default class TrashReports extends React.Component{
  ...
  handleList = () =>{
    let files = User.getFiles( "deleted" )
    ...
    if(files){
      ...
      Object.keys(files).reverse().forEach(k => {
        ...
        delet.setAttribute('class','recycle-file')
        delet.innerHTML = '<i class="far fa-recycle"></i>'
  ...
  render(){ 
    return(
      <div id="__report">
        <h4 className="title">
          <Language en="Excluded Reports" pt="Laudos Excluídos" />
        </h4>        
        <div id="directorys">
        </div>
      </div>
